...
// 更新组件的选中状态
function updateComponentsStatus(rects = [], checked = false) {
...
function getMatchedComponentsByLoc(x, y) {
    return [...drag._componentsMap.values()].filter((rect) => {
        const { width, height } = rect;
        const { x: rectX, y: rectY } = rect.$el.getBoundingClientRect();
...
// 碰撞判断
function getCollidingComponents(target) {
...
const createShap = (options = {}) => {
    ...
    const rect = creatRect(Object.assign(Object.assign({}, options), { sort }));
    const $components = getCollidingComponents(rect).sort((a, b) => a.layout - b.layout);
    if ($components.length > 0) {
        rect.layout = $components.pop().layout + 1;
    }
    else {
        rect.layout = 1;
    }
